...
# The test runs the test server and calls wget using the client's credentials
...
# Try to connect using wget
wget --private-key="pkcs11:token=test;object=test;type=private;pin-value=1234" \
                    --ca-certificate="$TESTDIR/ca/ca.crt" \
                    -O $TESTDIR/out https://localhost:5556
RV=$?
